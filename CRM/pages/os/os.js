...
function renderClients() {
    ...
    c.innerHTML = f.map(cl => `<div class="client-card" onclick="showClientDetail('${cl.phone}')"><div class="client-card-name">${cl.name||''}</div><div class="client-card-phone">📞 ${cl.phone||''}</div><div class="client-card-count">${(cl.history||[]).length} O.S.</div><div style="display:flex; gap:8px; margin-top:8px;">${podeEditar('os') ? `<button onclick="event.stopPropagation(); editClient('${cl.phone}')" style="background:none;border:none;cursor:pointer;font-size:16px;opacity:0.7;" onmouseover="this.style.opacity=1" onmouseout="this.style.opacity=0.7">✏️</button>` : ''}${podeExcluir('os') ? `<button onclick="event.stopPropagation(); deleteClient('${cl.phone}')" style="background:none;border:none;cursor:pointer;font-size:16px;opacity:0.7;" onmouseover="this.style.opacity=1" onmouseout="this.style.opacity=0.7">🗑️</button>` : ''}</div></div>`).join('');
}
