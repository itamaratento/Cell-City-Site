...
function goBack() {
  guardNavigation(() => { screenHistory.pop(); showScreen(screenHistory.length > 0 ? screenHistory[screenHistory.length - 1] : 'home'); });
}
...
function renderDetail() {
  ...
  hasUnsavedChanges = false; // ✅ Reset flag ao carregar nova OS
  ...
  html += `<div class="detail-header"><div class="detail-header-top"><div class="detail-os-id">${os.id}</div><span class="os-card-status status-${(os.status||'').replace(/ /g, '_')}">${getStatusLabel(os.status)}</span></div><div class="detail-client">${os.clientName} ${os.password ? `<span style="display:inline-flex;align-items:center;gap:4px;font-size:11px;color:#fbbf24;background:rgba(251,191,36,0.1);padding:2px 8px;border-radius:100px;">🔒 ${os.password}</span>` : ''}</div><div style="font-size:13px;color:var(--text2);margin-top:4px;">📞 ${os.phone}</div><div style="font-size:13px;color:var(--text2);margin-top:10px;padding-top:10px;border-top:1px solid var(--border);">📦 ${getCategoryIcon(os.category)} ${os.model}</div><div style="font-size:13px;color:var(--text2);margin-top:4px;">${os.defect || ''}</div></div>`;
  ...
  c.innerHTML = html;
  updateSaveUI(); // ✅ Renderiza estado inicial
}
...
// ✅ SALVAR OBSERVAÇÃO TÉCNICA (INTERNA)
...
function editClient(phone) { 
  guardNavigation(() => {
    ...
    window.markSaved(); // ✅ Reseta ao abrir editor
  });
}
