...
function initPatternCanvas() {
    ...
    const padding = 50;
    const spacing = (canvasSize - 2 * padding) / 2;
    ...
    patternCanvas.addEventListener('mouseout', stopDrawing);
    
    patternCanvas.addEventListener('touchstart', (e) => { e.preventDefault(); startDrawing(getTouchEvent(e)); });
    patternCanvas.addEventListener('touchmove', (e) => { e.preventDefault(); draw(getTouchEvent(e)); });
    patternCanvas.addEventListener('touchend', (e) => { e.preventDefault(); stopDrawing(); });
}
...
function startDrawing(e) {
    ...
    const point = getClosestPoint(e.offsetX, e.offsetY);
    if (point) {
        currentPath.push(point);
        patternSequence.push(point.index);
        drawPatternDots();
    }
}
...
function draw(e) {
    ...
    const point = getClosestPoint(e.offsetX, e.offsetY);
    if (point && !currentPath.includes(point)) {
        currentPath.push(point);
        patternSequence.push(point.index);
        drawPatternDots();
    }
...
function drawPatternDots() {
    ...
    if (currentPath.length > 1) {
        patternCtx.strokeStyle = '#00C853';
        patternCtx.lineWidth = 3;
        patternCtx.lineCap = 'round';
        patternCtx.lineJoin = 'round';
        patternCtx.shadowBlur = 10;
        patternCtx.shadowColor = 'rgba(0, 200, 83, 0.5)';
        
        patternCtx.beginPath();
        patternCtx.moveTo(currentPath[0].x, currentPath[0].y);
        for (let i = 1; i < currentPath.length; i++) {
            patternCtx.lineTo(currentPath[i].x, currentPath[i].y);
        }
        patternCtx.stroke();
        patternCtx.shadowBlur = 0;
    }
    
    for (const point of patternPoints) {
        const isConnected = currentPath.includes(point);
        const isLast = point === currentPath[currentPath.length - 1];
        
        patternCtx.beginPath();
        patternCtx.arc(point.x, point.y, isConnected ? 14 : 10, 0, Math.PI * 2);
        
        if (isLast) {
            patternCtx.fillStyle = '#00E676';
            patternCtx.shadowBlur = 15;
            patternCtx.shadowColor = 'rgba(0, 200, 83, 0.6)';
        } else if (isConnected) {
            patternCtx.fillStyle = '#00C853';
            patternCtx.shadowBlur = 8;
            patternCtx.shadowColor = 'rgba(0, 200, 83, 0.4)';
        } else {
            patternCtx.fillStyle = '#6b7280';
            patternCtx.shadowBlur = 0;
        }
...
function closePatternModal(event) {
    if (event && event.target.id !== 'pattern-overlay' && !event.target.closest('button')) return;
...
function savePattern() {
    if (patternSequence.length < 4) {
        showToast('⚠️ O padrão deve ter pelo menos 4 pontos');
        return;
    }
    ...
    // NÃO marcar como não salvo - apenas registrar padrão não é alteração pendente
}

function showPatternDrawing() {
    if (!window.tempPatternSequence || window.tempPatternSequence.length === 0) {
        showToast('⚠️ Nenhum padrão registrado');
        return;
    }
    ...
    setTimeout(() => {
        const previewCanvas = document.getElementById('pattern-preview-canvas');
        ...
        if (window.tempPatternSequence.length > 1) {
            previewCtx.strokeStyle = '#00C853';
            previewCtx.lineWidth = 3;
            previewCtx.lineCap = 'round';
            previewCtx.lineJoin = 'round';
            previewCtx.shadowBlur = 10;
            previewCtx.shadowColor = 'rgba(0, 200, 83, 0.5)';
            
            previewCtx.beginPath();
            const startPoint = points[window.tempPatternSequence[0]];
            previewCtx.moveTo(startPoint.x, startPoint.y);
            
            for (let i = 1; i < window.tempPatternSequence.length; i++) {
                const point = points[window.tempPatternSequence[i]];
        ...
        for (const point of points) {
            const isConnected = window.tempPatternSequence.includes(point.index);
            const isLast = point.index === window.tempPatternSequence[window.tempPatternSequence.length - 1];
            
            previewCtx.beginPath();
            previewCtx.arc(point.x, point.y, isConnected ? 14 : 10, 0, Math.PI * 2);
            
            if (isLast) {
                previewCtx.fillStyle = '#00E676';
                previewCtx.shadowBlur = 15;
                previewCtx.shadowColor = 'rgba(0, 200, 83, 0.6)';
            } else if (isConnected) {
                previewCtx.fillStyle = '#00C853';
                previewCtx.shadowBlur = 8;
                previewCtx.shadowColor = 'rgba(0, 200, 83, 0.4)';
            } else {
...
function showPatternSequence() {
    ...
    const sequence = window.tempPatternSequence.map(i => i + 1).join(' → ');
    openModal(`
        <div class="modal-handle"></div>
        <h3 style="font-size:16px; font-weight:700; margin-bottom:16px; color:var(--text); text-align:center;">
            🔢 Sequência do Padrão
        </h3>
        <div style="background:var(--surface2); padding:20px; border-radius:var(--radius); text-align:center; margin-bottom:16px;">
            <div style="font-size:24px; font-weight:800; color:var(--green-light); font-family:var(--font-mono);">
                ${sequence}
            </div>
            <div style="font-size:12px; color:var(--text3); margin-top:8px;">
                ${window.tempPatternSequence.length} pontos conectados
            </div>
        </div>
        <button type="button" onclick="closeModal()" style="width:100%; padding:12px; background:var(--green-primary); border:none; border-radius:var(--radius-sm); cursor:pointer; font-size:13px; color:#000; font-weight:700;">Fechar</button>
    `);
}
...
// CORREÇÃO: Removido window.markUnsaved() - selecionar tipo não é alteração pendente
function toggleLockType() {
    ...
    const lockPhotoSection = document.getElementById('lock-photo-section');
    const senhaInput = document.getElementById('f-senha');
    
    if (lockType === 'Padrao') {
        patternSection.style.display = 'block';
        lockPhotoSection.style.display = 'none';
        senhaInput.disabled = true;
        senhaInput.value = '';
    } else if (lockType === 'Numerica' || lockType === 'Biometria') {
        patternSection.style.display = 'none';
        lockPhotoSection.style.display = 'block';
    ...
    // NÃO chamar window.markUnsaved() aqui
}
...
async function showScreen(id) {
    guardNavigation(async () => {
        ...
        if (id === 'home') { 
            ...
            // CORREÇÃO: Resetar estado ao retornar ao dashboard
            window.markSaved();
...
// ===== CREATE OS =====
// CORREÇÃO: Adicionado window.markSaved() para resetar flag ao abrir nova OS
function startOS(cat) { 
    ...
    // CORREÇÃO: Resetar flag de alterações não salvas ao criar nova OS
    window.markSaved(); 
...
async function saveOS() {
    ...
    const os = { 
        ...
        password: lockType === 'Padrao' ? '' : senha, 
        lockType, 
        lockPhoto: currentLockPhoto, 
        photos: tempPhotos, 
        entryChecklist: entryChecked, 
        exitChecklist: [], 
        status: 'em_analise', 
    ...
    if (lockType === 'Padrao' && window.tempPatternSequence) {
...
function showOSPatternDrawing(osId) {
    ...
    if (!os || !os.patternSequence || os.patternSequence.length === 0) {
        showToast('⚠️ Nenhum padrão registrado');
        return;
    }
    ...
    setTimeout(() => {
        ...
        if (os.patternSequence.length > 1) {
            previewCtx.strokeStyle = '#00C853';
            previewCtx.lineWidth = 3;
            previewCtx.lineCap = 'round';
            previewCtx.lineJoin = 'round';
            ...
            previewCtx.beginPath();
            const startPoint = points[os.patternSequence[0]];
            previewCtx.moveTo(startPoint.x, startPoint.y);
            
            for (let i = 1; i < os.patternSequence.length; i++) {
                const point = points[os.patternSequence[i]];
                previewCtx.lineTo(point.x, point.y);
            }
            previewCtx.stroke();
            previewCtx.shadowBlur = 0;
        ...
        for (const point of points) {
            const isConnected = os.patternSequence.includes(point.index);
            const isLast = point.index === os.patternSequence[os.patternSequence.length - 1];
...
function showOSPatternSequence(osId) {
    ...
    const sequence = os.patternSequence.map(i => i + 1).join(' → ');
    openModal(`
        <div class="modal-handle"></div>
        <h3 style="font-size:16px; font-weight:700; margin-bottom:16px; color:var(--text); text-align:center;">
            🔢 Sequência do Padrão
        ...
        <div style="background:var(--surface2); padding:20px; border-radius:var(--radius); text-align:center; margin-bottom:16px;">
            ...
            <div style="font-size:12px; color:var(--text3); margin-top:8px;">
                ${os.patternSequence.length} pontos conectados
            </div>
        </div>
        <button type="button" onclick="closeModal()" style="width:100%; padding:12px; background:var(--green-primary); border:none; border-radius:var(--radius-sm); cursor:pointer; font-size:13px; color:#000; font-weight:700;">Fechar</button>
    `);
