...
function updateStats() {
  ...
  if (bar) {
    bar.innerHTML = `<div class="stat-chip"><span class="num">${orders.length}</span><span class="stat-label">Total</span></div>
      <div class="stat-chip"><span class="num">${andamento}</span><span class="stat-label">Em andamento</span></div>
      <div class="stat-chip"><span class="num">${finalizados}</span><span class="stat-label">Finalizados</span></div>`;
  }
...
function renderChecklist(containerId, items, key, checked = []) {
  ...
  container.innerHTML = items.map((item, i) => 
    `<div class="checklist-item"><input type="checkbox" id="${key}-${i}" ${checked.includes(i) ? 'checked' : ''} onchange="updateChecklistItem('${key}', ${i}, this.checked)"><label for="${key}-${i}" style="cursor:pointer;flex:1">${item}</label></div>`
  ).join('');
}
...
function renderList() {
  ...
  let filtered = currentListFilter === 'em_andamento' ? orders.filter(o => !['pronto','entregue'].includes(o.status)) : orders.filter(o => ['pronto','entregue'].includes(o.status));
  ...
  if (filtered.length === 0) { container.innerHTML = `<div class="empty-state"><div class="icon">${currentListFilter === 'em_andamento' ? '🔧' : '✅'}</div><p>${searchTerm ? 'Nenhum resultado encontrado' : 'Nenhuma O.S. nesta categoria'}</p></div>`; return; }
  container.innerHTML = filtered.map(os => `<div class="os-card" onclick="openDetail('${os.id}')"><div class="os-card-header"><span class="os-card-id">${os.id}</span><span class="os-card-status status-${os.status.replace(/ /g, '_')}">${getStatusLabel(os.status)}</span></div><div class="os-card-name">${os.clientName}</div><div class="os-card-info">${os.model} — ${os.defect.substring(0, 45)}${os.defect.length > 45 ? '...' : ''}</div><div class="os-card-footer"><span class="os-card-date">${formatDate(os.createdAt)}</span><span class="os-card-category">${getCategoryIcon(os.category)} ${getCategoryLabel(os.category).replace(/^.+\s/, '')}</span></div></div>`).join('');
}
