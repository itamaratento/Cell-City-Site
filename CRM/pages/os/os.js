...
// ===== EXPOSIÇÃO GLOBAL (Impede erro "is not defined" no HTML) =====
...
function removeLockPhoto() {
    ...
    const p = document.getElementById('lock-photo-preview'); if (p) p.innerHTML = '';
}

// ===== PATTERN LOCK FUNCTIONS (SENHA PADRÃO ANDROID) =====
...
function goBack() {
    guardNavigation(() => {
        screenHistory.pop();
        showScreen(screenHistory.length > 0 ? screenHistory[screenHistory.length - 1] : 'home');
    });
}
...
function formatPhone(v) {
    ...
    return v.length > 6 ? `(${v.slice(0,2)}) ${v.slice(2,7)}-${v.slice(7)}` : v.length > 2 ? `(${v.slice(0,2)}) ${v.slice(2)}` : v.length > 0 ? `(${v}` : v;
}
...
function updateStats() {
    ...
    if (bar) bar.innerHTML = `<div class="stat-chip"><span class="num">${orders.length}</span><span class="stat-label">Total</span></div><div class="stat-chip"><span class="num">${andamento}</span><span class="stat-label">Em andamento</span></div><div class="stat-chip"><span class="num">${finalizados}</span><span class="stat-label">Finalizados</span></div>`;
    ['andamento', 'finalizados'].forEach(type => {
        const el = document.getElementById(`badge-${type}`);
        if (!el) return;
        el.style.display = (type === 'andamento' ? andamento : finalizados) > 0 ? 'flex' : 'none';
        el.textContent = type === 'andamento' ? andamento : finalizados;
    });
}

// ===== CHECKLIST & PHOTOS =====
function getChecklistTemplate(cat) {
    const b = ['Tela / Display','Câmera frontal','Câmera traseira','Botões / Teclado','Carregador / Bateria','Wi-Fi / Bluetooth','Alto-falante','Microfone'];
    return cat === 'celular' ? [...b, 'Slot SIM/SD', 'Sensor de proximidade', 'Leitor digital']
        : cat === 'notebook' ? [...b, 'Trackpad', 'Portas USB', 'Leitor de cartão', 'Webcam']
        : cat === 'impressora' ? ['Cabeçote de impressão','Cartuchos / Toner','Bandeja de papel','Conexão USB/Wi-Fi','Display/Painel','Rolos de alimentação']
        : b;
}

function renderChecklist(cid, items, key, checked = []) {
    const c = document.getElementById(cid);
    if (!c) return;
    c.innerHTML = items.map((item, i) => `<div class="checklist-item"><input type="checkbox" id="${key}-${i}" ${checked.includes(i) ? 'checked' : ''} onchange="updateChecklistItem('${key}', ${i}, this.checked)"><label for="${key}-${i}" style="cursor:pointer;flex:1">${item}</label></div>`).join('');
}
...
async function updateChecklistItem(type, index, value) {
    ...
    value ? !currentOS[key].includes(index) && currentOS[key].push(index) : currentOS[key] = currentOS[key].filter(i => i !== index);
    await saveCurrentOS();
}
    ...
    for (let f of files) {
        ...
        r.onload = function(ev) {
            ...
            img.onload = function() {
                ...
                if (w > max || h > max) w > h ? (h = h * max / w, w = max) : (w = w * max / h, h = max);
                c.width = w; c.height = h; c.getContext('2d').drawImage(img, 0, 0, w, h);
                tempPhotos.push(c.toDataURL('image/jpeg', 0.7));
                renderPhotoPreview();
            };
            img.src = ev.target.result;
        };
...
function renderPhotoPreview() {
    ...
    if (!c) return;
    c.innerHTML = tempPhotos.map((p, i) => `<div class="photo-thumb-wrap"><img class="photo-thumb" src="${p}" onclick="viewPhoto('${p}')"><button class="photo-remove" onclick="removePhoto(${i})">✕</button></div>`).join('');
}

function removePhoto(i) { tempPhotos.splice(i, 1); renderPhotoPreview(); }

function viewPhoto(src) {
    openModal(`<div class="modal-handle"></div><img src="${src}" style="width:100%;border-radius:8px;">`);
}

// ===== CREATE OS =====
function startOS(cat) {
    currentCategory = cat;
    tempPhotos = [];
    currentLockPhoto = null;
    window.tempPatternSequence = null;
...
async function saveOS() {
    ...
    const os = {
        id: osId, category: currentCategory, clientName: nome, phone: telefone,
...
async function updateClientHistory(phone, name, osId) {
    let c = DB.getClients().find(cl => cl.phone === phone);
    if (c) { !c.history.includes(osId) && c.history.push(osId); c.name = name; }
    else { c = { name, phone, history: [osId] }; }
...
function renderList() {
    const orders = DB.getOS();
    const s = (document.getElementById('list-search')?.value || '').toLowerCase();
    const isFinal = currentListFilter === 'finalizados';
    let filtered = isFinal
        ? orders.filter(o => ['pronto','entregue'].includes(o.status))
        : orders.filter(o => !['pronto','entregue'].includes(o.status));
    ...
    if (filtered.length === 0) {
        c.innerHTML = `<div class="empty-state"><div class="icon">${isFinal ? '✅' : '🔧'}</div><p>${s ? 'Nenhum resultado encontrado' : 'Nenhuma O.S. nesta categoria'}</p></div>`;
        return;
    }
...
// ===== DETAIL =====
function openDetail(osId) {
    currentOS = DB.getOS().find(o => o.id === osId);
    if (currentOS) { renderDetail(); showScreen('detail'); }
}

function renderDetail() {
    const os = currentOS; const c = document.getElementById('detail-content');
    if (!os) return;
    hasUnsavedChanges = false;
    const statuses = [
        { key: 'em_analise', label: 'Em análise', color: 'var(--blue)' },
        { key: 'aguardando_peca', label: 'Aguardando peça', color: 'var(--yellow)' },
        { key: 'em_reparo', label: 'Em reparo', color: 'var(--orange)' },
        { key: 'pronto', label: 'Pronto', color: 'var(--green)' },
        { key: 'entregue', label: 'Entregue', color: 'var(--text3)' }
    ];
    const clients = DB.getClients();
    const client = clients.find(cl => cl.phone === os.phone);
    ...
    if (client?.history.length > 1) {
        ...
        html += `<div style="background:var(--surface2);border:1px solid var(--border);border-radius:var(--radius);padding:14px;margin-bottom:20px;"><div class="form-section-title" style="margin-bottom:8px;">📂 Histórico do Cliente</div>${otherOS.map(hId => {
            const h = DB.getOS().find(o => o.id === hId);
            return h ? `<div onclick="openDetail('${h.id}')" style="padding:10px 0;border-bottom:1px solid var(--border);display:flex;justify-content:space-between;align-items:center;cursor:pointer;"><div><div style="font-size:12px;font-weight:800;color:var(--green-light);">${h.id}</div><div style="font-size:11px;color:var(--text3);">${h.model} — ${formatDateShort(h.createdAt)}</div></div><span class="os-card-status status-${(h.status||'').replace(/ /g, '_')}">${getStatusLabel(h.status)}</span></div>` : '';
        }).join('')}</div>`;
    }
...
function renderChecklistHTML(key, items, checked, readonly) {
    return items.map((item, i) => `<div class="checklist-item"><input type="checkbox" ${checked.includes(i) ? 'checked' : ''} ${readonly ? 'disabled' : `onchange="updateChecklistItem('${key}', ${i}, this.checked)"`}><label style="cursor:${readonly ? 'default' : 'pointer'};flex:1">${item}</label></div>`).join('');
}
...
async function changeStatus(newStatus) {
    ...
    await saveCurrentOS(); renderDetail();
    showToast(`✅ ${getStatusLabel(newStatus)}`);
    window.markSaved();
}

async function saveObservation() {
    const t = document.getElementById('os-observations').value;
    if (!currentOS) return;
    currentOS.observations = t;
    await updateDoc(doc(db, "os", currentOS.id), { observations: t, updatedAt: new Date().toISOString() });
    showToast("✅ Observações salvas.");
    window.markSaved();
}

async function saveTechObservation() {
    const t = document.getElementById('os-tech-obs').value;
    if (!currentOS) return;
    currentOS.technicalObservation = t;
    await updateDoc(doc(db, "os", currentOS.id), { technicalObservation: t, updatedAt: new Date().toISOString() });
    showToast("🛠️ Nota técnica salva.");
    window.markSaved();
}
...
function addObservation() {
    ...
    currentOS.timeline.push({ date: new Date().toISOString(), text: `Nota: ${text}` });
    currentOS.updatedAt = new Date().toISOString();
    saveCurrentOS(); renderDetail();
...
function openClientFromOS() {
    if(currentOS) { currentClientPhone = currentOS.phone; showClientDetail(currentOS.phone); }
}
...
function renderClients() {
    ...
    if (f.length === 0) {
        c.innerHTML = `<div class="empty-state"><div class="icon">👥</div><p>${s ? 'Nenhum cliente encontrado' : 'Nenhum cliente cadastrado'}</p></div>`;
        return;
    }
...
function startOSForClient(phone, name) {
    startOS(DB.getOS().filter(o => o.phone === phone)[0]?.category || 'celular');
    document.getElementById('f-nome').value = name || '';
    document.getElementById('f-telefone').value = phone || '';
}
...
async function saveClientEdit(oldPhone) {
    ...
    try {
        await updateDoc(doc(db, "clientes", oldPhone), { name: n, phone: p });
...
async function deleteClient(phone) {
    ...
    try {
        ...
        if (e.code === 'not-found') { alert("Cliente não encontrado no banco."); }
        else { alert("Erro ao excluir do Firestore."); }
    }
}

// ===== GLOBAL SEARCH =====
function globalSearch() {
    const t = (document.getElementById('global-search')?.value || '').trim().toLowerCase();
    const c = document.getElementById('search-results');
    if (!c) return;
    if (t.length < 2) {
        c.innerHTML = `<div class="empty-state"><div class="icon">🔍</div><p>Digite pelo menos 2 caracteres</p></div>`;
        return;
    }
    const orders = DB.getOS().filter(o => (o.clientName||'').toLowerCase().includes(t) || (o.phone||'').includes(t) || (o.id||'').toLowerCase().includes(t) || (o.model||'').toLowerCase().includes(t));
    const clients = DB.getClients().filter(cl => (cl.name||'').toLowerCase().includes(t) || (cl.phone||'').includes(t));
    let h = '';
    if (clients.length > 0) h += `<div class="form-section"><div class="form-section-title">Clientes</div>${clients.map(cl => `<div class="client-card" onclick="showClientDetail('${cl.phone}')"><div class="client-card-name">${cl.name||''}</div><div class="client-card-phone">📞 ${cl.phone||''}</div></div>`).join('')}</div>`;
    if (orders.length > 0) {
