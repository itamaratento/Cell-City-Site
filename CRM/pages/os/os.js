...
// Botao de acao principal (Entregue / Devolver) no topo do detalhe
function _renderAcaoBtn(os) {
    if (!podeEditar('os')) return '';
    const aguardandoAprov = os.status === 'orcamento_enviado' || os.status === 'orcamento' || os.status === 'aguardando_aprovacao';
    if (aguardandoAprov) {
        return '<div style="margin:8px 0;"><button class="btn" onclick="markOrcamentoDevolvido()" style="background:#a78bfa;color:#000;font-weight:800;width:100%;padding:10px;">📋 Devolver Aparelho</button></div>';
    }
    if (!STATUS_TERMINAIS.includes(os.status)) {
        return '<div style="margin:8px 0;"><button class="btn btn-success" onclick="markDelivered()" style="width:100%;padding:10px;">📦 Entregue</button></div>';
    }
    return '';
}

// Quick-status: mostra os próximos status lógicos como botões de 1 clique
function _renderQuickStatus(os) {
    if (!podeEditar('os')) return '';
    const status = os.status || '';
    // Mapa de próximo status lógico
    const nextMap = {
        'recebido':             ['em_analise'],
        'em_analise':           ['orcamento_enviado'],
        'orcamento_enviado':    ['orcamento_aprovado', 'orcamento_recusado'],
        'orcamento_aprovado':   ['em_reparo'],
        'em_reparo':            ['testes_finais'],
        'testes_finais':        ['concluido'],
        'concluido':            ['entregue'],
    ...
    const nextKeys = nextMap[status] || [];
    if (!nextKeys.length) return '';
    const flow = STATUS_FLOW;
    return '<div class="quick-status-bar">' + nextKeys.map(k => {
        const s = flow.find(f => f.key === k);
        if (!s) return '';
        return '<button class="quick-status-btn" onclick="changeStatus(\'' + k + '\')" style="background:' + s.color + ';border:none;border-radius:6px;padding:4px 10px;cursor:pointer;font-size:11px;font-weight:700;color:#000;transition:opacity .15s;" onmouseover="this.style.opacity=.8" onmouseout="this.style.opacity=1">' + s.label + '</button>';
    }).join('') + '</div>';
}
...
function renderList() {
    ...
    if (s) filtered = filtered.filter(o => (o.clientName||'').toLowerCase().includes(s) || (o.phone||'').includes(s) || (o.id||'').toLowerCase().includes(s) || (o.model||'').toLowerCase().includes(s) || (o.imei||'').includes(s) || (o.imei1||'').includes(s) || (o.imei2||'').includes(s) || (o.brand||'').toLowerCase().includes(s));
...
function shareWhatsApp() { if(!currentOS) return; const os=currentOS; const text=`*Cell City - O.S.*\n📋 ${os.id}\n👤 ${os.clientName}\n📱 ${os.model}\n🔧 ${os.defect}\nStatus: ${getStatusLabel(os.status)}\n📅 ${formatDate(os.createdAt)}`; window.open(`https://wa.me/55${normalizePhoneDigits(os.phone)}?text=${encodeURIComponent(text)}`, '_blank'); }

// Abre o Portal do Cliente já autenticado no telefone desta OS, direto na
// própria OS (?tel=<phoneDigits>&os=<osId>) — evita a equipe pesquisar o
// cliente manualmente. Ver Portal.js::_boot() para o auto-login. O telefone
// é o próprio dado já gravado na OS (mesmo campo que a busca do Portal usa
// para localizar o cliente), não uma nova pesquisa por telefone digitado.
function abrirPortalCliente(osId, phoneDigits) {
...
function renderRetornoPanelHTML(os) {
    ...
    const _podeEditarOs = podeEditar('os');
    const checkboxesHtml = TIPOS.map(t => {
        ...
        return `<div class="retorno-checkbox-item" ${_podeEditarOs ? `onclick="marcarRetorno('${t.key}')"` : 'style="opacity:0.6;cursor:default;"'}>
    ...
    return `<div id="retorno-panel" class="retorno-panel" style="display:none;">
        ...
        ${_podeEditarOs ? `<div style="margin-top:10px;text-align:right;">
...
async function sendWarrantyWhatsApp() {
    ...
    const createdDate = formatDate(os.createdAt).split(' ')[0];
    const text = `*${lojaNome}*\n\nOS: ${os.id}\nData: ${createdDate}\nEquipamento: ${os.model}\nServiço: ${os.defect || 'Não especificado'}\n\n*GARANTIA*\n\n${garantia.texto}\n\nObrigado pela preferência.`;
    window.open(`https://wa.me/55${normalizePhoneDigits(os.phone)}?text=${encodeURIComponent(text)}`, '_blank');
...
window._executePrint = function() {
    ...
    w.document.write(`<!DOCTYPE html><html><head><title>${escHtml(os.id)}</title><style>body{font-family:monospace;padding:20px;max-width:400px;margin:0 auto}.row{display:flex;justify-content:space-between;padding:4px 0;font-size:12px}.label{font-weight:bold}.section{margin-top:12px;border-top:1px dashed #ccc;padding-top:6px}.footer{text-align:center;margin-top:20px;font-size:10px}@media print{button{display:none}}</style></head><body>${logoHtml}${cabecalhoHtml}<div class="row"><span class="label">O.S.:</span><span>${escHtml(os.id)}</span></div><div class="row"><span class="label">Data:</span><span>${formatDate(os.createdAt)}</span></div><div class="section"><div class="row"><span class="label">Cliente:</span><span>${escHtml(os.clientName)}</span></div><div class="row"><span class="label">Telefone:</span><span>${escHtml(os.phone)}</span></div>${os.cpf?`<div class="row"><span class="label">CPF:</span><span>${escHtml(os.cpf)}</span></div>`:''}${os.cep||os.endereco?`<div class="row"><span class="label">Endereço:</span><span>${escHtml([os.endereco, os.numero].filter(Boolean).join(', '))}${os.complemento?` - ${escHtml(os.complemento)}`:''}</span></div><div class="row"><span class="label">Bairro:</span><span>${escHtml(os.bairro||'')}</span></div><div class="row"><span class="label">Cidade/UF:</span><span>${escHtml([os.cidade, os.estado].filter(Boolean).join(' - '))}</span></div><div class="row"><span class="label">CEP:</span><span>${escHtml(os.cep||'')}</span></div>`:''}<div class="row"><span class="label">Aparelho:</span><span>${escHtml([os.brand, os.model].filter(Boolean).join(' '))}</span></div>${os.imei?`<div class="row"><span class="label">IMEI:</span><span>${escHtml(os.imei)}</span></div>`:''}<div class="row"><span class="label">Defeito:</span><span>${escHtml(os.defect||'')}</span></div>${os.valor?`<div class="row"><span class="label">Valor:</span><span>R$ ${Number(os.valor).toFixed(2)}</span></div>`:''}${os.patternSequence&&os.patternSequence.length?`<div class="row"><span class="label">Padrão:</span><span>${os.patternSequence.map(i=>i+1).join('→')}</span></div>`:''}${os.observations?`<div class="row"><span class="label">Obs:</span><span>${escHtml(os.observations)}</span></div>`:''}</div><div class="section">${statusHtml}</div>${garantiasHtmlAjustado}<div class="footer" style="margin-top:30px"><div style="text-align:center;margin-bottom:20px"><p style="margin:0;font-size:10px">Declaro ter recebido o aparelho e estar ciente dos termos de garantia.</p><p style="margin:30px 0 10px 0;border-top:1px solid #000;padding-top:10px;min-height:40px;font-size:11px;font-weight:bold">Assinatura do Cliente</p><p style="margin-top:15px;font-size:10px">Data da entrega: ___/___/____</p></div><div style="text-align:center;margin-top:20px;border-top:1px dashed #ccc;padding-top:10px;font-size:9px"><p style="margin:0;font-style:italic;color:#333">Confira todas as funcionalidades do aparelho antes de deixar a loja.</p></div></div></body></html>`);
    w.document.close();
    w.print();
};
...
function renderClients() {
    ...
    c.innerHTML = f.map(cl => `<div class="client-card" onclick="showClientDetail('${escHtml(cl.phone)}')"><div class="client-card-name">${escHtml(cl.name||'')}</div><div class="client-card-phone">📞 ${escHtml(cl.phone||'')}</div><div class="client-card-count">${(cl.history||[]).length} O.S.</div><div style="display:flex; gap:8px; margin-top:8px;">${podeEditar('os') ? `<button onclick="event.stopPropagation(); editClient('${escHtml(cl.phone)}')" style="background:none;border:none;cursor:pointer;font-size:16px;opacity:0.7;" onmouseover="this.style.opacity=1" onmouseout="this.style.opacity=0.7">✏️</button>` : ''}${podeExcluir('os') ? `<button onclick="event.stopPropagation(); deleteClient('${escHtml(cl.phone)}')" style="background:none;border:none;cursor:pointer;font-size:16px;opacity:0.7;" onmouseover="this.style.opacity=1" onmouseout="this.style.opacity=0.7">🗑️</button>` : ''}</div></div>`).join('');
}
...
function showClientDetail(phone) {
    guardNavigation(() => {
        ...
        let html = `<div class="detail-header" style="position:relative;">
            ${podeEditar('os') ? `<button onclick="editClientFromDetail('${phone}')" style="position:absolute;top:8px;right:8px;background:var(--surface3);border:1px solid var(--border);padding:6px 10px;border-radius:var(--radius-sm);cursor:pointer;font-size:12px;">✏️ Editar Ficha</button>` : ''}
            ...
            <div style="font-size:13px;color:var(--text2);margin-top:8px;">📞 ${client.phone || ''} ${client.phone2 ? `<span style="margin-left:10px;">📱 ${client.phone2}</span>` : ''} ${client.whatsapp && client.whatsapp !== client.phone ? `<span style="margin-left:8px;"><a href="https://wa.me/55${normalizePhoneDigits(client.whatsapp)}" target="_blank" style="color:#25D366;text-decoration:none;" title="WhatsApp">💬 ${client.whatsapp}</a></span>` : ''}</div>
...
function globalSearch() {
    ...
    if (t.length < 2) { c.innerHTML = `<div class="empty-state"><div class="icon">🔍</div><p>Digite pelo menos 2 caracteres</p></div>`; return; } const orders = DB.getOS().filter(o => (o.clientName||'').toLowerCase().includes(t) || (o.phone||'').includes(t) || (o.id||'').toLowerCase().includes(t) || (o.model||'').toLowerCase().includes(t) || (o.imei||'').includes(t) || (o.imei1||'').includes(t) || (o.imei2||'').includes(t) || (o.brand||'').toLowerCase().includes(t)); const clients = DB.getClients().filter(cl => (cl.name||'').toLowerCase().includes(t) || (cl.phone||'').includes(t)); let h = ''; if (clients.length > 0) h += `<div class="form-section"><div class="form-section-title">Clientes</div>${clients.map(cl => `<div class="client-card" onclick="showClientDetail('${escHtml(cl.phone)}')"><div class="client-card-name">${escHtml(cl.name||'')}</div><div class="client-card-phone">📞 ${escHtml(cl.phone||'')}</div></div>`).join('')}</div>`;
    if (orders.length > 0) {
