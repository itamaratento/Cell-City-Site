  ...
  async _boot() {
    ...
    if (saved) {
      try {
        ...
        console.log('[AUDIT:BOOT] currentOS antes do listener:', this.currentOS?.length, '(será 0 se listener nunca iniciou)');
        console.log('[AUDIT:BOOT] ATENÇÃO: _listenOS() NÃO é chamado no boot! currentOS permanecerá vazio até login.');
      } catch {
