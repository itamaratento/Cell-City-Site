...
window.Portal = {
  ...
  async enviarAvaliacao() {
    ...
    if (sessionStorage.getItem('portal_avaliou')) {
      this._toast('⏳ Você já enviou uma avaliação. Obrigado!');
      return;
    }
        ...
        <div class="contato-card" style="flex-direction:column;align-items:stretch;cursor:default;">
          <div style="padding:16px;">
            ...
            <p style="font-size:13px;color:#888;margin:0 0 12px 0;">
              Descreva o problema do seu aparelho e entraremos em contato com um orçamento.
            </p>
            <textarea id="solicitacao-desc" class="msg-textarea" placeholder="Ex: Celular não liga, tela trincada, notebook superaquecendo..." style="width:100%;box-sizing:border-box;min-height:80px;resize:vertical;"></textarea>
  ...
  renderComoChegar() {
    ...
    el.innerHTML = `
      <div class="como-chegar-container">
        ${osPronta.length > 0 ? `
          <a href="${MAPS_URL}" target="_blank" class="pronto-banner" style="text-decoration:none;display:flex;margin-bottom:12px;" onclick="window.Portal._registrarEvento('clique_maps',{pagina:'como-chegar',origem:'banner'})">
            <div class="pronto-banner-icon">🟢</div>
            <div class="pronto-banner-text">
              <strong>Seu aparelho está pronto para retirada!</strong>
  ...
  async _enviarSolicitacaoDiagnostico() {
    ...
    if (desc.length < 10) {
      this._toast('Descreva o problema (mínimo 10 caracteres)');
      descInput.focus();
      return;
    }
    ...
    try {
      ...
      this._toast('✅ Solicitação enviada com sucesso! Em breve entraremos em contato.');
      descInput.value = '';
    } catch (err) {
      console.error('[Portal] Erro ao enviar solicitação:', err);
      this._toast('❌ Erro ao enviar. Tente novamente.');
    } finally {
      btn.style.display = '';
      loading.style.display = 'none';
    }
