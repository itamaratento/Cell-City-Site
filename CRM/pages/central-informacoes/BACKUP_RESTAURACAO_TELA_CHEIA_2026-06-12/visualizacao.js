...
window.abrirTelaCheiaViewer  = abrirTelaCheiaViewer;
window.fecharTelaCheiaViewer = fecharTelaCheiaViewer;
...
function abrirViewer(id) {
    ...
    if (_modalTelaCheiaAberto()) {
        _preencherTelaCheia(info);
    }
...
// ===== TELA CHEIA =====
function abrirTelaCheiaViewer() {
    if (!_viewerIdAtivo) {
        _toast('⚠️ Selecione um registro primeiro');
        return;
    }
    ...
    const modal = document.getElementById('info-fullscreen-modal');
    if (!modal) return;
    modal.classList.add('active');
    modal.setAttribute('aria-hidden', 'false');
    document.body.classList.add('info-fullscreen-lock');
    document.querySelector('.info-fullscreen-panel')?.focus?.();
}

function fecharTelaCheiaViewer() {
    const modal = document.getElementById('info-fullscreen-modal');
    if (!modal) return;
    modal.classList.remove('active');
    modal.setAttribute('aria-hidden', 'true');
    document.body.classList.remove('info-fullscreen-lock');
}

function _modalTelaCheiaAberto() {
    return document.getElementById('info-fullscreen-modal')?.classList.contains('active');
}

function _preencherTelaCheia(info) {
    document.getElementById('ifs-icone').textContent      = _getIcone(info.tipo);
    document.getElementById('ifs-titulo').textContent     = info.titulo || '(sem título)';
    document.getElementById('ifs-categoria').textContent  = info.categoria || '';
    document.getElementById('ifs-tipo-badge').textContent = _getTipoLabel(info.tipo);
    document.getElementById('ifs-corpo').innerHTML        = _renderCorpo(info);
    document.getElementById('ifs-rodape').innerHTML       = _renderRodape(info);
    document.getElementById('ifs-corpo').scrollTop = 0;
}
...
document.addEventListener('keydown', (event) => {
    if (event.key === 'Escape' && _modalTelaCheiaAberto()) {
        fecharTelaCheiaViewer();
    }
});
