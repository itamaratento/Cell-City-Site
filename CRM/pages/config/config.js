...
function handleLogoUpload(e) {
    ...
    if (!file) return;
    if (file.size > 500000) {
        showToast('⚠️ Imagem muito grande. Máximo 500kb.');
        return;
    }
...
function addGarantia() {
    ...
    syncGarantias();
    config.garantias.push({ id, nome: 'Nova garantia', texto: '' });
    renderGarantias();
...
async function saveConfig() {
    ...
    try {
        const data = { logo: config.logo || '', garantias: config.garantias || [], updatedAt: serverTimestamp() };
        await setDoc(CONFIG_DOC, data);
        localStorage.setItem(CACHE_KEY, JSON.stringify({ logo: data.logo, garantias: data.garantias }));
