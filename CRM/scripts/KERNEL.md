...
cb(user)
   ...
   │                              initTenant(ctx, empresaId)  ← tenant-provider (PS-1/PS-2)
...
_ready resolve (uma única vez — _readyResolved trava novas resoluções)
        ...
        │
        ▼
initModulo() (chamado por cada módulo) — Promise.race(_ready, timeout 10s) + clearTimeout
   ├─ ctx resolvido → retorna ctx para o módulo
   └─ timeout/sem ctx → location.href = loginUrl() (preserva prefixo /dev) e retorna null

logout() → signOut → clearTenant() → remove flag → redirect login
```

Pontos-chave da consolidação (Fase 1.3 + fechamento 2026-07-23 em `develop`):
...
- **Um único `onAuthStateChanged` persistente** por página no Kernel — o
  ...
  Kernel.
- **Tenant via `tenant-provider`** após `_buildContext` (não reintroduzir
  `EMPRESA_ID` hardcoded — isso reverteria PS-1/PS-2).
...
- **Timeout único** (`TIMEOUT_MS = 10_000`) com `clearTimeout` após o race.
