...
(function () {
  ...
  function buildHTML() {
    ...
    const itemsHTML = ITEMS.map(item => {
      ...
      return `<a class="${classes}" href="${ENV_PREFIX}${item.href}" data-sid="${item.id}" data-tip="${item.label}" draggable="true">
        <span class="cc-drag-handle">⠿</span>
        <span class="cc-si-icon">${item.icon}</span>
        <span class="cc-si-label">${item.label}</span>
  ...
  function setupCentralModulosBtn() {
    ...
    btn.addEventListener('click', () => {
      ...
      window.location.href = ENV_PREFIX + '/CRM/pages/central-modulos/index.html';
    });
  }
