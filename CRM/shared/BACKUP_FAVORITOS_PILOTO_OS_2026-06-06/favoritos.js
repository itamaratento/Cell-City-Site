/* ============================================================
   ...
   • Armazenamento: localStorage (chave cc_favoritos).
   • Não toca em Firebase, Firestore Rules, Auth nem no Portal.

   Carregado automaticamente em todas as páginas que importam
   shared/dock.js, e diretamente no Dashboard via <script>.
   ============================================================ */

const STORAGE_KEY = 'cc_favoritos';
...
function saveFavoritos(list) {
  ...
  // Avisa outras abas/instâncias para re-renderizarem
...
function renderLauncher() {
  ...
  pinBtn.innerHTML = fixado
    ? `<span>✓</span><span>Fixado</span>`
    : `<span>📌</span><span>Fixar nos Favoritos</span>`;
...
function renderDropdown(dd) {
  const favs = loadFavoritos();
  dd.innerHTML = `<div class="ccfav-dd-title">Acesso rápido</div>`;
  if (favs.length === 0) {
    const e = document.createElement('div');
    e.className = 'ccfav-dd-empty';
  ...
  favs.forEach(fav => {
    ...
    item.className = 'ccfav-dd-item';
    item.innerHTML = `<span class="ccfav-ico">${fav.icon || '📌'}</span><span class="ccfav-dd-txt"></span><span class="ccfav-x" title="Remover">✖</span>`;
    item.querySelector('.ccfav-dd-txt').textContent = fav.label;
    item.addEventListener('click', (e) => {
      if (e.target.classList.contains('ccfav-x')) return;
      navigate(fav.url);
    });
    item.querySelector('.ccfav-x').addEventListener('click', (e) => {
      e.stopPropagation();
      removeFavorito(fav.key);
    });
    dd.appendChild(item);
