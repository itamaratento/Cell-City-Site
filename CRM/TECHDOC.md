...
**Atualização — rotação executada (mesmo dia, sessão de continuação, autorizada explicitamente pelo dono):** as 2 chaves comprometidas foram desabilitadas (uma 3ª chave nova gerada e validada em seu lugar); confirmado por inventário e por tentativa real de autenticação com a chave antiga (falhou). Suítes revalidadas pós-rotação: 77/77, sem regressão. Detalhe completo na Fase 7 de `plans/AUDITORIA_GERAL_20260706.md`. Exclusão definitiva das chaves (irreversível) ainda pendente de decisão do dono — o checklist de pré-deploy passa a ter só esse item como bloqueio residual (desabilitada já é considerado "não mais ativa" para efeito de neutralização do vazamento).
