...
function renderDetail() {
    ...
    if (_orcResp === 'aprovado' || _orcResp === 'recusado') {
        ...
        html += `<div style="background:${_bg};border:1px solid ${_cor};border-radius:var(--radius);padding:14px;margin-bottom:16px;"><div style="font-weight:800;color:${_cor};font-size:14px;">${_txt}</div>${_meta.length ? `<div style="font-size:12px;color:var(--text2);margin-top:6px;line-height:1.7;">${_meta.join(' &nbsp;•&nbsp; ')}</div>` : ''}</div>`;
    }
...
async function saveOSEdit() {
    ...
    try {
        const updates = { clientName: n, phone: p, cpf: cpf || null, cep: cep || null, endereco: endereco || null, numero: numero || null, complemento: complemento || null, bairro: bairro || null, cidade: cidade || null, estado: estado || null, brand, model: m, imei, defect, technician: tecnico, valor, valorCartao, prazoGarantia, garantiaId: garantiaId || null, password, observations, updatedAt: new Date().toISOString() };
