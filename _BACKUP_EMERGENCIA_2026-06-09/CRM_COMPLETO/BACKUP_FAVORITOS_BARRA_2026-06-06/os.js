...
async function init() {
    ...
    await DB.loadFromFirestore(); updateStats(); showScreen('home'); updateFavStars();
    // Prioridade: deep-link da barra de favoritos (#fav-...) > favorito de abertura padrão (estrela)
    const hashView = getHashView();
    if (hashView) openFav(hashView);
    else { const fav = getFav(); if (fav) openFav(fav); }
...
// Deep-link da barra de favoritos quando já estamos na página de OS (hash muda sem recarregar)
window.addEventListener('hashchange', () => { const v = getHashView(); if (v) openFav(v); });