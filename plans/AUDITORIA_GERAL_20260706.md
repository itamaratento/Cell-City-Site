...
**Credencial administrativa (service account) de PRODUÇÃO vazada em commit antigo de repositório público permanecia ATIVA — nunca rotacionada, conhecida desde 2026-07-03.** ~~Ambas ainda ativas e sem expiração~~ **ATUALIZADO (Fase 7): as 2 chaves foram DESABILITADAS em 2026-07-06**, autorizado explicitamente pelo dono, com uma 3ª chave nova gerada e validada em seu lugar — ver detalhe completo na Fase 7 abaixo. A service account tem papéis de administrador de Cloud Functions, Cloud Run, Firebase Auth e Admin SDK completo (bypass de todas as Firestore Rules) — nível de acesso mais amplo do que se sabia antes desta auditoria. Detalhe técnico completo (IDs das chaves, comando de remediação passo a passo) em `plans/AUDITORIA_GERAL_20260706_INTERNO.md`. **Pendência residual: exclusão definitiva das 2 chaves (irreversível) — desabilitação já neutraliza o vazamento na prática, exclusão fica para decisão explícita separada do dono.**
...
|---|---|---|---|
| 1 | Credencial admin vazada | 🔴 Crítico → 🟡 Mitigado | **Desabilitada (Fase 7)** — exclusão definitiva pendente de decisão |
...
7. **Decisão consciente de não excluir definitivamente ainda**: desabilitar já neutraliza o vazamento na prática (a chave vazada não pode mais ser usada por quem a tiver, e a service account não pode reabilitá-la sozinha) — a exclusão definitiva é irreversível e fica para uma decisão explícita separada do dono, seguindo a mesma janela de segurança já recomendada no plano original do INTERNO.
