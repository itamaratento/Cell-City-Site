...
# nunca estoura a moldura.
_cc_box_line() {
  local texto="$1" largura pad
  largura="$(_cc_box_width)"
  if [ "${#texto}" -gt "$largura" ]; then
...
# Texto centralizado — usado só pro título e pro subtítulo (versão/fase).
_cc_box_line_center() {
  local texto="$1" largura total esq dir
  largura="$(_cc_box_width)"
  if [ "${#texto}" -gt "$largura" ]; then
    texto="${texto:0:$largura}"
  fi
