...
const AGENDAMENTO_STATUS = {
  ...
  'cancelado':   { label: 'Cancelado',              cor: '#EF5350', icon: '❌' },
  'atendido':    { label: 'Atendido',               cor: '#78909C', icon: '🎉' }
};
...
window.Portal = {
    ...
    el.innerHTML = `
      <div class="painel-container">
        ...
        <div class="painel-grid">
          <div class="painel-card painel-card-destaque" onclick="Portal.navegar('agendar')">
            <div class="painel-card-icon">📅</div>
            <div class="painel-card-title">Agendar Atendimento</div>
            <div class="painel-card-sub">Reserve um horário</div>
          </div>
  ...
  renderOSDetalhe(osId) {
    ...
    if (o.status === 'orcamento_aprovado' || o.orcamentoResposta === 'aprovado') {
      html += `
        <div class="orcamento-resposta aprovado" style="background:rgba(0,200,83,0.1);border:1px solid rgba(0,200,83,0.4);border-radius:12px;padding:14px;margin-bottom:12px;">
          <div style="font-weight:700;color:#00C853;">✅ Orçamento aprovado pelo cliente.</div>
    ...
    // ===== DOCUMENTOS (Relatório Técnico — só aparece se autorizado pelo técnico) =====
    ...
    if (relVisivel) {
      const secoesRel = [
        ['🗣️ Defeito informado', rel.defeitoInformado],
        ['🔍 Diagnóstico técnico', rel.diagnostico],
        ['🛠️ Solução aplicada', rel.solucaoAplicada],
        ['📌 Observações', rel.observacoes]
      ].filter(s => s[1]).map(s => `
        <div style="margin-bottom:14px;">
          <div style="font-size:12px;font-weight:700;text-transform:uppercase;letter-spacing:0.4px;color:#00A86B;margin-bottom:4px;">${this._esc(s[0])}</div>
          <div style="font-size:14px;color:#333;line-height:1.6;white-space:pre-wrap;">${this._esc(s[1])}</div>
        </div>`).join('');
      const relData = (rel.data && /^\d{4}-\d{2}-\d{2}$/.test(rel.data)) ? rel.data.split('-').reverse().join('/') : (rel.data ? this._fmtDate(rel.data) : '');
      html += `
        <div class="os-detail-section">
          <h3 class="os-detail-section-title">📄 Documentos</h3>
          <div class="doc-item" onclick="Portal.toggleRelatorio()" style="display:flex;align-items:center;gap:12px;padding:14px;border:1px solid #e0e0e0;border-radius:12px;cursor:pointer;background:#fafafa;">
            <span style="font-size:22px;">📋</span>
            <div style="flex:1;">
              <div style="font-weight:700;color:#222;">Relatório Técnico</div>
              <div style="font-size:12px;color:#888;">${this._esc([o.brand, o.model].filter(Boolean).join(' '))}${relData ? ' • ' + relData : ''}</div>
            </div>
            <span id="doc-rel-arrow" style="color:#888;font-size:18px;">›</span>
          </div>
          <div id="doc-rel-body" style="display:none;margin-top:12px;padding:16px;border:1px solid #e0e0e0;border-radius:12px;background:#fff;">${secoesRel}</div>
        </div>
      `;
    }
  ...
  // Abre/fecha o conteúdo do Relatório Técnico no Portal
  toggleRelatorio() {
    const body = document.getElementById('doc-rel-body');
    const arrow = document.getElementById('doc-rel-arrow');
    if (!body) return;
  ...
  async aprovarOrcamento(osId) {
    ...
    try {
      ...
      await updateDoc(ref, {
        ...
        ...this._orcamentoCampos('aprovado'),
        updatedAt: serverTimestamp(),
        timeline: arrayUnion({
          date: new Date().toISOString(),
          text: 'Orçamento aprovado pelo cliente (Portal do Cliente)'
        })
      });
  ...
  async recusarOrcamento(osId) {
    ...
    try {
      ...
      await updateDoc(ref, {
        ...
        timeline: arrayUnion({
          date: new Date().toISOString(),
          text: `Orçamento recusado pelo cliente (Portal do Cliente)${motivo ? ': ' + motivo : ''}`
        })
      });
          ...
          ${ags.map(a => {
            ...
            return `
              <div class="msg-item">
                ...
                ` : ''}
                ${a.status === 'cancelado' && a.observacaoAdmin ? `
                  <div class="msg-item-resposta">
                    <div class="msg-resposta-header">❌ Cancelado</div>
                    <div class="msg-resposta-text">${this._esc(a.observacaoAdmin)}</div>
                  </div>
                ` : ''}
