...
test('LIMITES.consulta_os_publica é mais restrito que leitura', () => {
  assert.equal(LIMITES.consulta_os_publica.ip, 8);
  assert.ok(LIMITES.consulta_os_publica.ip < LIMITES.leitura.ip);
});

test('consulta_os_publica: permite até 8 req/min no mesmo IP', () => {
...
test('consulta_os_publica: a 9ª req no mesmo IP → resource-exhausted', () => {
  const req = fakeRequest();
  for (let i = 0; i < 8; i++) aplicarRateLimit(req, 'consulta_os_publica');
  assert.throws(
...
test('consulta_os_publica: IPs distintos não compartilham o contador', () => {
  for (let i = 0; i < 8; i++) aplicarRateLimit(fakeRequest('198.51.100.1'), 'consulta_os_publica');
